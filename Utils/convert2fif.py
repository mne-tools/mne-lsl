...
def pcl2fif(filename, interactive=False, outdir=None, external_event=None):
	...
	"""
	fdir, fname, fext= qc.parse_path(filename)
	if outdir is None:
		outdir= fdir + 'fif/'
	elif outdir[-1] != '/':
		outdir += '/'
	...
	# fix wrong event channel
		...
		qc.print_c('Warning: Specified event channel (%d) != inferred event channel (%d).'% (trig_ch,trig_ch_guess), 'Y')
		if interactive: raw_input('Press Enter to fix. Event channel will be set to %d.'% trig_ch_guess)
...
def bdf2fif(filename, interactive=False, outdir=None):
	...
	if raw.info['chs'][-1]['ch_name'] != 'STI 014':
		qc.print_c("*** ERROR: The last channel (%s) doesn't seem to be an event channel. Entering debugging mode."% raw.info['chs'][-1]['ch_name'] )
		pdb.set_trace()
...
def any2fif(filename, interactive=False, outdir=None, ch_name_file=None):
	...
	if fext == 'pcl':
		pcl2fif(filename, interactive=interactive, outdir=outdir)
	elif fext == 'eeg':
