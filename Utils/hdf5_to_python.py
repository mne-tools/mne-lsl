...
for rawfile in qc.get_file_list(DATA_DIR, fullpath=True):
	...
	diff_idx = np.ravel(np.nonzero(timestamps-timestampsOffset)) # non-zero element of this are the one we seek,[0] because it's stupid to have a nparray inside a tuple
