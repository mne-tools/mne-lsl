...
if __name__ == '__main__':
    ...
    if pickle.HIGHEST_PROTOCOL >= 4:
        ...
        tm = qc.Timer()
        print('Reading images from %s' % LEFT_IMAGE_DIR )
        left_images = read_images(LEFT_IMAGE_DIR)
        print('Reading images from %s' % RIGHT_IMAGE_DIR)
        right_images = read_images(RIGHT_IMAGE_DIR)
        print('Took %.1f s. Start exporting ...' % tm.sec())
        ...
        print('Your Python pickle protocol version is less than 4, which will be slower with loading a pickle object.')
