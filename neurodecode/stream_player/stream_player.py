...
class Streamer:
    ...
    def stream(self, repeat=np.float('inf'), high_resolution=False):
        ...
        while played <= repeat:
            ...
            self._logger.debug('[%8.3fs] sent %d samples (LSL %8.3f)' % (
    ...
    def set_lsl_info(self, server_name):
        ...
        desc.append_child('amplifier').append_child(
            'settings').append_child_value('is_slave', 'false')
        desc.append_child('acquisition').append_child_value(
            'manufacturer', 'NeuroDecode').append_child_value('serial_number', 'N/A')
    ...
    def get_trg_index(self):
        ...
        """
        return find_event_channel(raw=self.raw)

    def get_info(self):
        """
        Log the info about the created LSL stream.
        """
        self._logger.info(f'Server name: {self.server_name}')
        self._logger.info(
            f'Sampling frequency {self.get_sample_rate() = :.3f} Hz')
        self._logger.info(f'Number of channels : {self.get_nb_ch()}')
        self._logger.info(f'Chunk size : {self.chunk_size}')
        for i, ch in enumerate(self.raw.ch_names):
            self._logger.info(f'{i} {ch}')
        self._logger.info(f'Trigger channel : {self.get_trg_index()}')
                ...
                if self._tdef is None:
                    ...
                    for event in event_values:
                        if event in self._tdef.by_value:
                            ...
                            self._logger.info(
                                f'Events: {event} (Undefined event)')
...
if __name__ == '__main__':
    ...
    if not fif_file.exists() or fif_file.suffix not in ['.fif', '.fiff']:
