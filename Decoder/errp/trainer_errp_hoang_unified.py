...
"""
#Instructions
# This script can either work from a CLI or directly by itself.
# If working from terminal, the data path, channel set, filter method and subject ID are loaded as arguments.
# Search for "[CLI]" to jump to the relevant part of the code.
#
# Currently the output directory of the various plot are hardcoded to DIRECTORY_TO_WRITE_TO.
#
# If launched by itself (e.g. from an IDE like Spyder)
...
# If you want to modify some aspect of the created classifier, look for "[createMagic]"
"""
####################################
...
THATGUY = 'D:/Hoang/My Documents/Python/ErrP/q5/super500/'
...
from triggerdef_errp import TriggerDef
...
def compute_features(signals, dataset_type, sfreq, l_freq, h_freq, decim_factor, shiftFactor, scaleFactor, pca, tmin,
    ...
    dataset_type: string
        ...
        If train, this function output the scale and shift factor as well as the PCA object.
        If test, it uses the scale and shift factor given as parameter (same thing with PCA)
    l_freq, h_freq: float
    ...
    # Apply PCA and keep component that explains 95% of the variance
    if dataset_type == 'train':
        pca = PCA(0.95)
...
def normalizeAcrossEpoch(epoch_data, method, givenShiftFactor=0, givenScaleFactor=1):
    ...
    if len(
            new_epochs_data.shape) == 3:  # this is a bit dangerous ! dirty hack to make this function work with 2D data as well
...
def preprocess(loadedraw, events, \
               ...
               baselineRange, \
               verbose=False):
    # Load raw and its properties,CAR if asked to do so, bandpass if asked by filter_method, epochs the data
    # and return the epochs, its properties and the filter properties
...
def processCV(loadedraw, \
              ...
              FILTER_METHOD, \
              verbose=False):
    # From raw, load data, preprocess, then apply cross validation
    ...
    if (cv_container is None):
        cv_container = []  # the different folds for CV are first generated then put into a container for further analysis
    ...
    for train_x, test_x, train_label, test_label in cv_container:
        cls = rLDA(regcoeff)  # init rLDA classifier
        cls.fit(train_x, train_label)  # Train classifier
        # Alternative classifier init
        ...
        # cls = QDA(reg_param=0.3) # regularized LDA (from sklearn)

        predicted = cls.predict(test_x)  # test
...
if __name__ == '__main__':
    ...
    for var_padding in var_padding_range:
        ...
        for var_regcoeff in var_regcoeff_range:
            ...
            auc_result, threshold, cm, best_tpr, best_fpr, cv_container, biglist_cms, roc_fprs, roc_tprs = processCV(
                loadedraw, \
                events, \
                tmin=COMMON_TMIN, \
                tmax=COMMON_TMAX, \
    ...
    f = open(DIRECTORY_TO_WRITE_TO + '/' + SUBJECT_ID + '-results-trainingCV-' + str(THATCHANNELSET) + '-' + str(
        THATFILTERMETHOD) + '-' + OTHER + '.txt', 'w')
    ...
    plt.colorbar()
    if SAVEFIGURE:
        plt.savefig(DIRECTORY_TO_WRITE_TO + '/' + SUBJECT_ID + '-grid-' + 'AUC-' + str(THATCHANNELSET) + '-' + str(
            THATFILTERMETHOD) + '-' + OTHER + '.png')
        plt.savefig(DIRECTORY_TO_WRITE_TO + '/' + SUBJECT_ID + '-grid-' + 'AUC-' + str(THATCHANNELSET) + '-' + str(
            THATFILTERMETHOD) + '-' + OTHER + '.svg')
    # plt.close()
    ...
    plt.colorbar()
    if SAVEFIGURE:
        plt.savefig(DIRECTORY_TO_WRITE_TO + '/' + SUBJECT_ID + '-grid-' + 'TP-' + str(THATCHANNELSET) + '-' + str(
            THATFILTERMETHOD) + '-' + OTHER + '.png')
        plt.savefig(DIRECTORY_TO_WRITE_TO + '/' + SUBJECT_ID + '-grid-' + 'TP-' + str(THATCHANNELSET) + '-' + str(
            THATFILTERMETHOD) + '-' + OTHER + '.svg')
    # plt.close()
    ...
    plt.figure()
    plt.plot(rocs_fprs_np[best_tprs_idx[0][0], best_tprs_idx[1][0]],
             rocs_tprs_np[best_tprs_idx[0][0], best_tprs_idx[1][0]])
    plt.xlabel('False positive rate')
    plt.ylabel('True positive rate')
    plt.title('ROC Curve for best TP')
    if SAVEFIGURE:
        plt.savefig(DIRECTORY_TO_WRITE_TO + '/' + SUBJECT_ID + '-ROC-' + 'TP-' + str(THATCHANNELSET) + '-' + str(
            THATFILTERMETHOD) + '-' + OTHER + '.png')
        plt.savefig(DIRECTORY_TO_WRITE_TO + '/' + SUBJECT_ID + '-ROC-' + 'TP-' + str(THATCHANNELSET) + '-' + str(
            THATFILTERMETHOD) + '-' + OTHER + '.svg')
    ...
    if SAVEFIGURE:
        plt.savefig(DIRECTORY_TO_WRITE_TO + '/' + SUBJECT_ID + '-ROC-' + 'AUC-' + str(THATCHANNELSET) + '-' + str(
            THATFILTERMETHOD) + '-' + OTHER + '.png')
        plt.savefig(DIRECTORY_TO_WRITE_TO + '/' + SUBJECT_ID + '-ROC-' + 'AUC-' + str(THATCHANNELSET) + '-' + str(
            THATFILTERMETHOD) + '-' + OTHER + '.svg')
