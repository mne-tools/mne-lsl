...
def plot_grand_average(epochs,singleWindow=True, title='', alpha=0.05, correction='All',save=False):
    ...
    for channel in range(epochs._data.shape[1]):
        ...
        mne_reject_fdr_bh_tmp, mne_pval_fdr_tmp = mne.stats.fdr_correction(pval[channel,:], alpha=alpha, method='indep')
    ...
    for chidx in range(len(epavg[epavg.keys()[0]].data)):
        ...
        #Set the decorations
        plt.xticks([to_idx(0),to_idx(50),to_idx(100),to_idx(200),to_idx(300),to_idx(500),to_idx(800),to_idx(1000)],['0', '50', '100', '200', '300','500','800','1000'],rotation=-90)
        plt.xlabel('Time [ms]')
