	...
	if DO_CV:
		for train, test in cv:
			...
			if SIMULATE_ONLINE is True:
				...
				train_bp = train_data
			(train_normalized,trainShiftFactor,trainScaleFactor) = normalizeAcrossEpoch(train_bp,'MinMax') #normalization

			train_downsampling = train_normalized[:,:,::decim_factor] #downsampling

			train_reshaped = train_downsampling.reshape(train_downsampling.shape[0],-1) # merge channel and time for the pca
			pca = PCA(0.95)
