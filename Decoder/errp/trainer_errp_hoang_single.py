...
import pycnbi_utils as pu # sometime it doesn't work
...
def compute_features(signals, sfreq, l_freq, h_freq, decim_factor,shiftFactor,scaleFactor,pca,apply_online,tmin,tmax,paddingIdx,iir_params):
...
if __name__=='__main__':
	...
	epochs = mne.Epochs(raw, events=events, event_id=event_id, tmin=t_lower, tmax=t_upper,baseline=baselineRange, picks=picks_feat, preload=True, proj=False)
	if(baselineRange):
	...
	def apply_cv(epochs):
		...
		for train, test in cv:
			...
			if FILTER_METHOD is 'WINDOWED': # epochs should have one epoch only
				train_bp= mne.filter.band_pass_filter(train_data, sfreq, Fp1=2, Fp2=h_freq,copy=True,filter_length=None, method='fft',iir_params=None) #bandpass on one epoch
...
#			train_pcaed = train_reshaped
			...
			test_pcaed = compute_features(test_data,sfreq,l_freq,h_freq,decim_factor,trainShiftFactor,trainScaleFactor,pca,FILTER_METHOD,tmin,tmax,paddingIdx,iir_params=dict(a=a,b=b))
...
#				print('True negative rate: '+str(cm_normalized[1][1]))
	...
	if OUTPUT_PCL:
		...
		train_bp= mne.filter.band_pass_filter(train_data, sfreq, l_freq, h_freq, method='iir',iir_params=dict(a=a,b=b)) #bandpass
		train_bp= train_bp[:,:,paddingIdx:paddingIdx+(int((tmax-tmin)*sfreq))]
		...
		if False:
			pca= None
			X = compute_features(train_data_normalized,sfreq,l_freq,h_freq,decim_factor,trainShiftFactor,trainScaleFactor,pca)
			# Classifier init
			RF = dict(trees=100, maxdepth=None)
			cls = RandomForestClassifier(n_estimators=RF['trees'], max_features='auto', max_depth=RF['maxdepth'], n_jobs=n_jobs)
			cls.fit(X, label)
		...
		data= dict(cls=cls, sfreq=raw.info['sfreq'], ch_names=ch_names, picks=picks_feat,\
			...
			shiftFactor=trainShiftFactor, scaleFactor=trainScaleFactor, pca=pca,threshold=best_threshold[0],tmin=tmin, tmax=tmax,paddingIdx=paddingIdx,iir_params=dict(a=a,b=b))
