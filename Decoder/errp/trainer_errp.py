...
if __name__=='__main__':
	...
	if EXPORT_PLOTS==True:
		...
		# apply baseline
		epochs= mne.Epochs(raw, events=events, event_id=event_id, tmin=tmin, tmax=tmax, baseline=(None,0), picks=picks_feat, preload=True)
	...
	epochs= mne.Epochs(raw, events=events, event_id=event_id, tmin=tmin, tmax=tmax, baseline=None, picks=picks_feat, preload=True)
	...
	if DO_CV==True:
		...
		for train, test in cv:
			...
			(test_data_normalized,testShiftFactor,testScaleFactor) = normalizeAcrossEpoch(test_data,'override',trainShiftFactor,trainScaleFactor)

			### Downsampling
			train_data_downsampled = train_data_normalized[:,:,::decim_factor]
			test_data_downsampled = test_data_normalized[:,:,::decim_factor]

			train_x = train_data_downsampled.reshape(train_data_downsampled.shape[0],-1) #put the last dimension into the preceding one
			test_x = test_data_downsampled.reshape(test_data_downsampled.shape[0],-1) #put the last dimension into the preceding one
