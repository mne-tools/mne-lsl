...
if __name__=='__main__':
	...
	processing_steps = []
	def getperf(APPLY_CAR, APPLY_PCA, APPLY_OVERSAMPLING, DO_CV, l_freq,h_freq, picks_feat, offset, tmin, tmax,baselineRange,reg_coeff,verbose):
		raw = loadedraw.copy()
		if APPLY_CAR:
	...
	#        print(foo)
		...
		epochs = mne.Epochs(raw, events=events, event_id=event_id, tmin=tmin, tmax=tmax,baseline=baselineRange, picks=picks_feat, preload=True, proj=False, verbose = False)
		...
		def apply_cv(epochs):
			...
			for train, test in cv:
				...
				if SIMULATE_ONLINE is True: # epochs should have one epoch only
					train_bp= mne.filter.band_pass_filter(train_data, sfreq, l_freq, h_freq, method='iir' ) #bandpass on one epoch
				else:
					train_bp = train_data
	...
	#			train_pcaed = train_reshaped
				...
				test_pcaed = compute_features(test_data,sfreq,l_freq,h_freq,decim_factor,trainShiftFactor,trainScaleFactor,pca)
...
#		results_regcoeff.append([avg_confusion_matrixes,my_auc])
	...
	for var_tmax in var_tmax_range:
		...
		for var_regcoeff in var_regcoeff_range:
			...
			biglist_auc,biglist_cms,best_threshold, besttpr = getperf(APPLY_CAR, APPLY_PCA, APPLY_OVERSAMPLING, DO_CV,\
										l_freq,h_freq, picks_feat, offset, tmin, tmax,\
										baselineRange,reg_coeff=regcoeff, verbose=False)
	...
	best_results_cmat = cmats_np[best_results_idxs[0][0],best_results_idxs[1][0]] # take the first best results because... no reasons 
